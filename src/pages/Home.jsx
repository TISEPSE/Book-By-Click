  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/session", { credentials: "include" })
      ...
      .then(data => {
        if (data) {
          setIsLoggedIn(true)
          setEstGerant(Boolean(data.estGerant))
        }
      })
      .catch(() => {})
  }, [])
          ...
          {isLoggedIn ? (
            ...
            <>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-6">
                Bienvenue sur Book By Click
              </h2>
              <p className="text-lg sm:text-xl text-gray-700 mb-10 max-w-2xl mx-auto">
                {estGerant
                  ? "Gérez vos créneaux, suivez vos réservations et analysez votre activité depuis votre tableau de bord."
                  : "Trouvez un professionnel et réservez votre prochain rendez-vous en quelques secondes."}
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Link
                  to={estGerant ? "/dashboard_entreprise" : "/dashboard_client"}
                ...
                >
                  {estGerant ? "Mon tableau de bord" : "Mes réservations"}
                </Link>
                <Link
                  to="/result"
                  className="w-full sm:w-auto px-8 py-4 bg-white border-2 border-gray-300 text-gray-900 font-bold rounded-lg shadow-md hover:shadow-lg hover:border-indigo-600 hover:text-indigo-600 hover:-translate-y-0.5 transition-all duration-300"
                >
